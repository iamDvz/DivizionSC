...
> **Полная ёмкая версия:** [SHPARGALKA.md](SHPARGALKA.md)  
...
│   ├── defs-examples.dsc   # ← создаётся автоматически
...
- `defs-examples.dsc` — примеры: heal, fireball, blink, босс-комбо, ModelEngine VFX
...
Скриптовый синтаксис для **def**, **module** (переиспользуемые блоки) и **effect** (alias module).  
Файлы: `defs/defs-*.dsc`.

> **Канонические рабочие примеры** (покрыты тестами и создаются при первом запуске):
> `defs/defs-examples.dsc` (база), `defs/defs-advanced.dsc` (сложное), `defs/defs-fx-examples.dsc` (EffectLib/VFX).
>
...
>
> **Эффекты** — вызовы `verb(args)`: `heal(6)`, `damage(5, target)`, `sound(blaze_shoot, volume=0.8)`.
> Короткая форма `heal 6` тоже принимается.
>
> **Блочные конструкции пишутся со скобками:** `if (cond) { } else { }`, `chance (30%) { }`,
> `after(20t) { }`, `loop(times=3, interval=10) { }`, `area(6) { }`, `aura(radius=5, ...) { }`,
...
(сверено с тестами). Рабочие примеры всего перечисленного — в `defs/defs-examples.dsc`,
...
Указывают, на кого/откуда действует эффект. `->` и `→` — псевдонимы `>>`.
...
Если маршрут не указан — действует **умолчание по эффекту**: `heal`→self, `damage`→target,
`particle`/`sound`→точка цели и т.д. Так что маршрут нужен только чтобы переопределить умолчание.

## 10. Модули и аргументы
...
- Модули видны во всех файлах (общий индекс), не только в своём.
...
Module из `.dsc` видны другим def **в том же файле** (сверху вниз).

---

## Эффекты

Каждая строка в `do { }` — одна команда.
...
`heal`, `fireball`, `blink`, `bump`, `combo_strike`, `arcane_combo`, `hybrid_demo`, `boss_slam`, `boss_volley`, `boss_enrage`, `boss_chain_lightning`, `meg_flamethrower`, `meg_ground_slam` + модули.

Тест: `/dsc cast arcane_combo`, `/dsc cast boss_slam`
